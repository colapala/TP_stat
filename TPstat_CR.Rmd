 ...
 * Sois $\alpha = 5\%$
  ...
  * L'analyse de $a$, permet d'établir un premier résultat quantifiant la significativité du modèle. En effet nous allons tester la significativité de $a$ via le test statistique: $(H_0) : a = 0$ contre $(H_1) : a \neq 0$. La p-value de celui-ci ce retrouve dans le tableau summary(temps.lm) et est 
  
```{r, echo=FALSE}
summary(temps.lm)$coef[,"Pr(>|t|)"][2]
```
  
   Nous pouvons donc rejeter $H0$  et affirmer avec 5% de risque que $a$ est significatif. 
  * L'analyse de $b$ est la moins importante. Il nous indique seulement l'importance de l'intercept. Le test statistique est analogique à $a$. Sa p-value est
...
```
  ...
  Nous pouvons donc rejeter $H0$  et affirmer avec 5% de risque que $b$ est significatif.  
...
```
On ne peut pas rejetter $H_0$, donc nous pouvons affirmer avec un risque de 5% que les résidus suivent une loi normale. Ce qui indique un modèle pertinent. 
