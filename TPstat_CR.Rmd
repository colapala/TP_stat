...
Le résultat de ces tests est représenté par une lettre dans la colonne cld du tableau ci-dessous. Une même lettre est attribuée aux méthodes pour lequelles H0 n'est pas rejetée. Deux méthodes ayant des lettres différentes ont des temps d'exécution moyens différents.  
...
summary(temps.lm)
```
Nous pouvons remarquer qu'il y a une relation linéaire entre $\log(temps)²$ et $n$, puisque le test de Fisher ne rejette pas le modèle linéaire. De plus, $R²$ =
...
On peut donc en déduire que Branch&Bound semble avoir une complexité temporelle en $\exp(n)$.
