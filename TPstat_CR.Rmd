...
(Pour celui là je suis pas sûr que ce soit cette méthode qu'il faille utiliser !)

Vérification

```{r, echo=TRUE}
t <- t.test(cheminNearest,mu=mean(cheminBranch) ,alternative = "greater") #alternative = "greater" pour H1 m > m0
print(t)
```
On peut aussi faire t.test(cheminNearest, cheminBranch,alternative = "greater")
...
Si on accepte de se tromper de alpha=5%, on rejette H0 si la pvaleur de (i,j) est inférieure à alpha/10 (vu dans l'énoncé donc 0.05).
? pas sûr de ça !
...
```{r, echo=TRUE}
#lm(log(data.graph$tps)~., data = data.graph)
```
